...
static uint64_t load_program(struct task_struct* task) {
    ...
    for (int i = 0; i < phdr_cnt; i++) {
        ...
        if (phdr->p_type == PT_LOAD) {
            //printk("quanxianwei:%d\n",phdr->p_flags);
            uint64 addr=phdr->p_vaddr;
            uint64 size = (phdr->p_offset + phdr->p_memsz) / PGSIZE + 1;
            uint64 flag = (phdr->p_flags&0x111)<<1;
            uint64 content_offset=phdr->p_offset;
            do_mmap(task, phdr->p_vaddr, size*PGSIZE, flag, phdr->p_offset, phdr->p_filesz);
      }
    }
...
struct vm_area_struct *find_vma(struct task_struct *task, uint64_t addr){
    //printk("find_vma的addr=%lx\n",addr);
    for(int i=0;i<task->vma_cnt;i++){
        if(addr>=task->vmas[i].vm_start){
            if(addr<=task->vmas[i].vm_end){
                //printk("find_vma找到了,vm_start=%lx,vm_end=%lx\n",task->vmas[i].vm_start,task->vmas[i].vm_end);
                return &(task->vmas[i]);
            }
        }
    }
